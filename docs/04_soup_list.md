...
| SOUP-017 | `python:3.11-slim` base image | tag, not digest | Container base for the API service | NFR-001 | No | **A tag is not a version.** `python:3.11-slim` resolves to different image contents over time, including different OS package versions. This defeats reproducible container builds; pinning by digest is the remedy. §6 open item 3 |
| SOUP-014 | Orthanc | 24.7.3 (`orthancteam/orthanc:24.7.3`) | Local PACS — test and demonstration environment only | SRS-043, SRS-044, SRS-045 | No | Does not ship as part of the device software. It is the DICOMweb peer against which SRS-043..SRS-045 are exercised, so a defect in it can cause a passing round-trip test to mean less than it appears to |
...
| 3 | **SOUP-017 is pinned by tag, not by digest.** `python:3.11-slim` is mutable. Container builds are therefore not reproducible, which sits badly against NFR-002. Pin by digest, or record the non-reproducibility as accepted. | Milestone 7 |
