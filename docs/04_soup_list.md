...
| 9 | Nothing checks that the image digests recorded in SOUP-014 and SOUP-017 still match `docker/Dockerfile.api` and `docker/docker-compose.yml`. They can drift silently. **Allocated as TC-106** in `docs/07` §8. | `docs/07` |
