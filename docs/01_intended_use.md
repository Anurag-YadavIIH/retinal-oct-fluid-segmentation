...
| L2 | **Severe class imbalance.** Fluid occupies **0.60–1.48%** of voxels, measured (`docs/06` §3.2). Accuracy is meaningless here — a null predictor scores 98.52–99.40% — and is never reported (URS-007). **58 of 70 volumes lack at least one fluid class** (`docs/06` §3.2.1), so a per-class figure may rest on far fewer than 70 volumes and must be read with its sample size. |
| L3 | **Appearance shift and prevalence shift are not separable.** Measured cross-vendor prevalence ratios are 6.0× for SRF, 2.8× for IRF and 1.2× for PED (`docs/06` §3.2), so a cross-vendor gap cannot be attributed to appearance alone — least of all for SRF. The converse also holds and is newly visible: a PED gap across vendors **cannot** be explained by prevalence, since PED is nearly uniform. |
