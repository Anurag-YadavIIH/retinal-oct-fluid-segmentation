...
allocates URS-001..URS-010. It deliberately claims nothing about performance
...
| URS-003 | Every output shall be a candidate for grader review. The software shall take no automated action, shall not record or release a result on its own authority, and shall require an explicit grader action before any result enters a record. | The workflow in §1 is a pre-read. **This control must not be treated as fully mitigating in `docs/05` or `docs/03`:** automation bias is well documented, and a plausible-looking wrong segmentation is more likely to be accepted than an obviously blank one. Human review lowers the probability of an undetected error; it does not eliminate it, and the safety argument must account for the residual. |
