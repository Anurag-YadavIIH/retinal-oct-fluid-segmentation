...
| 2 | ~~Integration testing (62304 §5.6) not planned.~~ **Resolved 2026-09-17:** `docs/07` §5 defines the integration strategy and allocates TC-110..TC-113. Unimplemented. | — closed |
| 3 | ~~Unit verification process and acceptance criteria not recorded.~~ **Resolved 2026-09-17:** `docs/07` §3 carries the process and §4 the acceptance criteria A1..A6. Original text: `docs/07` as scoped is a **system-level** V&V protocol; it owes unit-level acceptance criteria and a statement of what unit verification (§5.5.5) must cover. A passing `pytest` suite is evidence of verification having been run, not the process or the criteria that govern it. | `docs/07` |
