...
# --- TC-039: the cache cannot change a value -------------------------------------------


def test_TC_039_cached_frames_are_identical_to_uncached(tmp_path):
    from monai.data import Dataset, PersistentDataset

    from ocuval.data.datamodule import prepare_cache

    records = [
        {
            "frame_id": f"TRAIN00{v}#{i:04d}",
    ...
    pixels = {r["frame_id"]: rng.integers(0, 255, (64, 32)).astype(np.uint8) for r in records}
    decodes: dict[str, int] = {}

    def load(record):
        out = dict(record)
        decodes[record["source_path"]] = decodes.get(record["source_path"], 0) + 1
        out["image"] = pixels[record["frame_id"]]
        return out
    ...
    uncached = Dataset(records, transform=chain)
    reference = [uncached[i]["image"].clone() for i in range(len(records))]

    decodes.clear()
    cache_dir = tmp_path / "cache"
    prepare_cache(records, chain, cache_dir)
    cached = PersistentDataset(records, transform=chain, cache_dir=str(cache_dir))
    served = [cached[i]["image"] for i in range(len(records))]

    for expected, actual in zip(reference, served, strict=True):
        assert torch.equal(expected, actual), (
            "a frame served from the cache differs from the uncached one. A cache is an "
            "optimisation and must not be able to change a value (SRS-078)."
        )


def test_TC_039_prepass_visits_each_volume_contiguously(tmp_path):
    """One decode per volume rather than one per frame is the whole point.

    Asserted as contiguity of visit order, which is what makes a per-volume decode
    possible, rather than by counting decodes through a cache whose hit behaviour would
    make the count depend on MONAI's internals.
    """
    from ocuval.data.datamodule import prepare_cache

    records = [
        {
            "frame_id": f"v{v}#{i:04d}",
            "source_path": f"/raw/vol{v}",
            "frame_index": i,
            "intensity_window": (0.0, 1.0),
        }
        for i in range(3)
        for v in range(3)  # deliberately interleaved: frame-major, not volume-major
    ]
    seen: list[str] = []

    def record_order(record):
        seen.append(record["source_path"])
        return dict(record)

    prepare_cache(records, record_order, tmp_path / "cache")

    # Each volume must appear as one unbroken run.
    runs = [key for index, key in enumerate(seen) if index == 0 or seen[index - 1] != key]
    assert len(runs) == len(set(runs)) == 3, f"volumes were revisited: {seen}"
