"""Verification of the DICOMweb client internals — TC-083..TC-086.
...
These carry their own test case identifiers rather than borrowing TC-080..TC-082. An
earlier version reused those, which made three PACS round-trip cases look executed when
only their client-side half ran — a coverage figure that flattered the project exactly
the way `docs/09` exists to prevent.
"""
...
def test_TC_084_instance_url_is_built_from_the_three_uids(client):
    url = client.instance_url("1.2.3", "4.5.6", "7.8.9")
    assert url == f"{BASE}/studies/1.2.3/series/4.5.6/instances/7.8.9"


def test_TC_084_studies_and_series_urls(client):
...
def test_TC_084_missing_uid_is_rejected(client, study, series, sop, missing):
...
def test_TC_084_multipart_body_has_opening_and_closing_boundaries():
    body = dw.build_multipart([b"PAYLOAD"], "abc123")
    assert body.startswith(b"--abc123\r\n")
    assert body.endswith(b"--abc123--\r\n")


def test_TC_084_each_part_declares_its_media_type_and_length():
...
def test_TC_084_lines_are_crlf_delimited():
...
def test_TC_084_missing_boundary_raises_rather_than_guessing(header):
...
def test_TC_085_store_response_reports_referenced_instances():
    payload = {
        "00081199": {
...
def test_TC_085_store_response_carries_failures_rather_than_discarding_them():
...
def test_TC_085_empty_store_response_is_handled():
    result = dw.parse_store_response(200, {})
    assert result.referenced_sop_instance_uids == []
    assert result.ok


def test_TC_085_store_requires_instances(client):
...
def test_TC_086_client_exposes_no_mutating_operation():
...
def test_TC_086_client_issues_only_get_and_post():
